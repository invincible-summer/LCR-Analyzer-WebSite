...
**精调语义（2026-09 R4-R5 + R16，`AlgorithmLcr/OPTIMIZATION_LOG.md`）**：Try2 的用户输入
...
含野点数据上上述比较改在稳健目标/稳健尺度上进行（R16：野点对 plain rss 是共同
常数项，会把真改善掩蔽进波动带；轴中位数尺度 1.4826×median 同 IRLS）。
三引擎均含离群点稳健重拟合（IRLS 迭代至不动点 ≤2 遍，5σ 降权；Try1 的渐近特征
另有污染触发的 Theil-Sen/圆中位数稳健估计，R15），含野点的实测数据不再被拉偏。
