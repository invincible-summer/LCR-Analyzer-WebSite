...
2000 组随机 DUT 战役：拟合达噪声底率 99.85%，良态案例参数中位误差 0.11%）
...
4. 高 $Q$ 谐振使目标景观出现指数窄的谷（相对宽度 $\sim 1/(2Q)$），纯随机多起点
   会以 ~1% 概率失手；谐振检测种子 + 拓扑感知 LC 配对种子 + 阻尼同伦救援把
   失手率压到 ~0.1–0.15%（§5.4，战役实测）。
...
2. **局部极小的实践可控性**：$\chi^2$ 非凸，但三层证据支持多起点策略足够：
   ...
   拓扑配对；深谷 → 同伦），2000 组仅 3 例（0.15%）未达 3× 底；
...
单案例中位 ~0.1 s、p99 ~2 s（含全部救援阶段）；每案例 ~29 个起点。
...
2. **电学等价族**（T5）：约 1.8% 的良态案例参数落在等价实现上（曲线在
   ...
   职责，列为 Try3 的 P2 扩展。
3. **高 Q 多 tank 深谷**：0.15% 案例多起点未达底（§5.4）；带外推不可信。
...
|---|---|
| 运行错误 | 0 / 2000 |
| **fit_ok（达 3× 噪声底以内）** | **99.85%**（3 例病态：0.052 / 0.30 / 0.60） |
| wRMSE p50 / p90 / p99 | 0.00687 / 0.00776 / 0.00855（底 0.00707） |
| 结构可辨识（满秩）占比 | 66.2% |
| curve_ok（可辨识案例） | **99.0%** |
...
| 单案例耗时 | 中位 113 ms、p90 0.68 s、p99 2.0 s |
...
**残留失败（3/2000）剖析**：均从真值出发可收敛（可达性实验），
但 137 个起点仍未命中——多 tank + 秩亏的复合病态；作为 0.15% 的
已知边界记录在 §8。
