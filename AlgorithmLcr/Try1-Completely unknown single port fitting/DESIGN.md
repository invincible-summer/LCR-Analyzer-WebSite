...
## 11. v2：实电感模型（L + DCR 绑定）与器件数约束（2026-09-04）

P1 之后的模型升级：**电感不再视为理想元件**。实验室电感的绕线直流电阻不可
忽略（mH 级典型 $\Omega$ 量级），且"$R + L$ 串联"与"带 DCR 的电感"在端口上
**电气不可区分**——v1 把它们当成两个不同的 2 器件电路，既是物理失真，又造成
器件计数歧义。v2 与 Try2（`Component(kind, value, dcr)`）、Try3
（`theta` 切片 `[log L, log Rd]`）的器件模型对齐。

### 11.1 器件模型

- 叶子仍是 $R / L / C$ 三种**器件**；$R$、$C$ 各 1 个参数，$L$ 器件 2 个参数
  $\theta = (\log_{10} L,\ \log_{10} R_d)$，阻抗 $Z_L = R_d + sL$；
- 器件数 = 叶子数（`n_leaves`），参数数 = 叶子数 + L 叶子数（`n_params`）；
  AICc 的 $K = p + 1$ 自动计入 DCR 自由度（数据不支持时简约惩罚会压掉它）；
- DCR 搜索域 $\log_{10} R_d \in [-6, 7]$（$1\,\mu\Omega \sim 10\,M\Omega$，
  与 Try3 `PHYS_BOUNDS` 一致）；下界 $R_{d,\min}=10^{-6}\,\Omega$ 即"理想"
  电感的规范代表（log10 参数化不能取 0）。

### 11.2 规范规则的修改（R2' 与 R4）

**R2'（并联多 L）**：v1 的 R2 把同节点同型叶子一律合并（理想电感下
$L_1 \| L_2 = L_1L_2/(L_1+L_2)$ 成立）。v2 中两个 $(L+R_d)$ 并联是
**二阶系统**（分子二次、零点 $-R_{d,i}/L_i$ 各异），不可合并：

$$Z = \frac{(R_{d1}+sL_1)(R_{d2}+sL_2)}{R_{d1}+R_{d2}+s(L_1+L_2)} \neq R_d + sL.$$

因此 PAR 节点允许**多个 L 叶子**（$L\|L$、$C\|L\|L$ 等成为新拓扑类）；
串联方向同型叶仍合并（$L$ 与 $R_d$ 均可加）。同型 $R/R$、$C/C$ 在串、并联
两个方向仍合并（电导/电容可加，一阶恒成立）。

**R4（串联吸收）**：$\forall R,\ R_d,\ L:\quad R + (R_d + sL) \equiv
(R_d + R) + sL$，即 SER 节点中 $R$ 叶与 $L$ 叶共存时，$R$ 折入 $L$ 器件的
DCR，**器件数减一**。注意 R4 只作用于同一 SER 节点的**直接**子叶（R1 展平
后）；$R + (L\|C)$ 中 $R$ 与 $L$ 不直接串联，不吸收。这与 Try3 的 F4 归约
（串联 R 吸收进相邻 L 的 rd）完全同构。

规范形仍满足合流性（每条规则保 $Z(s)$ 不变且减少"尺寸"度量），canonical
串唯一标识等价类；`normalize` 幂等性由测试锁定。

### 11.3 枚举计数（深度 ≤ 2，测试锁定）

| $n$（器件数） | 1 | 2 | 3 | 4 | 5 | 6 |
|---|---|---|---|---|---|---|
| v2（实电感） | 3 | 6 | 22 | 45 | 87 | 162 |
| v1（理想 L） | 3 | 6 | 20 | 36 | 54 | 78 |

深度 3、$n=4$：v2 99（v1 90）。变化来源：并联多 L 类（$P(L,L)$、
$P(C,L,L)$、…）新增，串联 $R{+}L$ 型被 R4 吸收删去。

### 11.4 器件数约束 `exact_n`（可选先验）

- 输入：可选 `count.txt`（单行正整数 $n$，见 `../../INPUT_FORMAT.md` §2.1）
  或 `Config(exact_n=n)` / CLI `--exact-n`；
- 语义：电路器件数**恰好**为 $n$（$L$+DCR 算 1 个）。生效时：
  引擎 A 的库取单层 `library.trees_of_size(n)`（`max_n` 被忽略）；引擎 B
  的 Foster 候选中器件数 $\neq n$ 的不进入选择器排序（极点信息仍用于 F3
  剪枝与起点注入）；F2/F3 剪枝照常作用于该层；
- 缺省 = 自由搜索 $1..n_{\max}$（v1 行为）。

### 11.5 引擎 B 的 Foster 映射修改

- Z 侧 $d + e\,s$ 两项共存 ⟹ **单个 L 器件** $[L{=}e,\ R_d{=}d]$（R4）；
- Y 侧实极点支路 $\rho'/(s+a')$ ⟹ 单个 L 器件 $[L{=}1/\rho',\ R_d{=}a'/\rho']$
  （v1 是独立的 $R$ 叶 + $L$ 叶）；"支路电阻低于带底"的丢弃分支取消——
  DCR 直接挂在 L 器件上，无害且更精确；
- Y 侧共轭对支路 ⟹ $\mathrm{SER}(L(R_d),\ C)$；条件仍是 $c'=0$（Y 侧没有
  对应的两器件族）；
- **Z 侧共轭对新增第二个可实现族**（v2 关键扩展）：部分分式对
  $\frac{As+B}{s^2+2\alpha s+\omega_n^2}$ 中，除 v1 的无损槽路族
  （$c=0 \Rightarrow R\|L\|C$）外，**有损槽路族** $(R_d+sL)\|C$ 恒满足
  $c = 2\alpha\rho_r$，闭式解 $C=\frac{1}{2\rho_r}$、$L=\frac{2\rho_r}{\omega_n^2}$、
  $R_d=\frac{4\alpha\rho_r}{\omega_n^2}$（2 器件 3 参数）。两族都不匹配
  （超出噪声感知容差）才是 Bott–Duffin 情形（D8 跳过）。
  这修复了 v2 的关键回归：有损双峰 DUT（dut8）在 v1 中靠 Foster 起点注入
  收敛，v2 数据物理上 $c\neq 0$ 后 v1 映射被 D8 正确拒绝、起点消失，
  引擎 A 多起点在 7 参数多峰景观上无法独立命中全局盆地（对称简并吸引子
  wrmse≈0.5）；有损槽路族使 Foster-I 再次精确综合出
  $S(P(C,L(R_d)),P(C,L(R_d)),R)$ 并作为起点注入。

### 11.6 v2 DUT 套件（§8.2 的替换，14 个）

原套件中 $R{+}L$ 串联型 DUT 在 v2 下塌缩为单 L 器件；所有电感赋真实 DCR；
新增并联多 L 类。器件值按可辨识性调优（见 §11.7）：

1. 单器件：$R=100\,\Omega$；**$L=1\,\mathrm{mH}, R_d=5\,\Omega$**；$C=10\,\mathrm{nF}$；
2. 串联二器件：$R{+}C$；$L(R_d{=}5) + C$（串联谐振 ~1.6 MHz，$Q{\approx}20$
   ——凹口深度须高于 0.5% 噪声底，$R_d$ 才可辨识）；
3. 并联二器件：$R\|C$；$R \| L(R_d{=}2)$；
4. 电感寄生：$L(R_d{=}1\,\Omega) \| C_p{=}50\,\mathrm{pF}$（2 器件）；
5. 电容寄生：$L_{\mathrm{esl}}(R_d{=}0.05) + C$（2 器件）；
6. 弛豫混联：$R_1 + (R_2\|C)$（3 器件，不变）；
7. 槽路：$R \| L(R_d{=}0.5) \| C$（3 器件）；
8. 双峰：$R + (L_1(R_d{=}10)\|C_1) + (L_2(R_d{=}20)\|C_2)$（5 器件，
   moderate $Q$ —— $R_d$ 的阻尼效应须高于 0.5% 噪声底才可辨识）；
9. **新类**：$L(10\,\mathrm{mH}, 5\,\Omega) \| L(1\,\mathrm{mH}, 50\,\Omega)$
   ——并联双电感；零点 500/5e4 rad/s、极点 5e3，彼此相距一个十倍频程
   （若零极点近乎对消则大电感不可辨识）；
10. **新类**：$R + (L_1 \| L_2)$——SER 链内嵌多 L PAR 节点（3 器件）；
    零点 1e4/3e7、极点 3e5，带内"零-极摆动"比噪声底高一个量级。

### 11.7 可辨识性发现（测试阈值的设计依据）

1. **dut8 损耗拆分模糊**：串联 $R$ 与槽路电感 $R_d$ 同为串联通路上的损耗，
   其拆分只有噪声级分辨率——0.5% 噪声下参数误差 ~5%（有噪阈值放宽至 8%，
   拓扑恢复不受影响）。
2. **dut10 全局参数退化**：$S(P(L,L),R)$ 族 5 参数只映射到 4 个阻抗不变量
   （串联 $R$ 可与电感 $R_d$ 连续交换，$R{+}P(L_1(R_1),L_2(R_2))$ 与
   $R'(R_1')\dots$ 同族），因此响应级恢复精确但元件值不可比——该 DUT
   跳过参数断言，仅断言拓扑/等价类。这正是 T2（拓扑不可辨识）的参数化
   形式，v1 中 dut6_relaxation 的等价类行为同源。
3. **可互换兄弟组**：canonical 串相同的兄弟（PAR 下两个 L 叶、两个
   结构相同的子树）在规范序中不可区分，拟合器可能对调其参数块——
   `max_param_error` 在这些组内取最优排列（置换感知比较）。

### 11.8 对外契约的同步

- 输出：L 边携带拟合 DCR（`Edge("L", L, Rd)`，不再恒 0）；同槽多条 L 边
  合法（重边），见 `../../OUTPUT_FORMAT.md` §5.1；
- 输入：新增可选 `count.txt`，见 `../../INPUT_FORMAT.md` §2.1；
- demo：`--exact-n N` / `--count PATH` / `--measurements PATH`（从
  measurements.txt 起跑的完整输入模式）。
