...
原生 theory 在输入频点直接求值；未来网页适配器负责扩展绘图频栅。
非有限诊断和无效 AICc 输出 JSON `null`，不用 `1e999` 或巨大有限哨兵。
原生报告不冒充已有网站响应协议；根 DESIGN.md 单独冻结后续 C ABI 边界。
...
浏览器适配使用原生 JSON。每个 `groups` 条目同时带 `u/v/kind/value/dcr`，明确对应拟合边；`members` 保持输入边索引，不能按邻接矩阵输出次序反推。
