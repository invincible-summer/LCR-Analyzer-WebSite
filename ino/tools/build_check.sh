...
#   * 固定验证组合：Arduino-ESP32 3.3.11 + TFT_eSPI 2.5.44。
#     TFT_eSPI 2.5.43 在 ESP32-S3 默认路径把 SPI_PORT 设为 Arduino FSPI；
#     Arduino-ESP32 3.x 在 S3 上定义 FSPI=0，而 TFT_eSPI 的 direct-register
#     路径需要 SPI2 外设号 2，会在 tft.init() 触发 EXCVADDR=0x10 的
#     StoreProhibited。2.5.44 上游已修复 S3 默认 SPI_PORT=2；display.cpp
#     另有 compile-time gate，防止依赖漂移重新引入该故障。
...
#     冲突），映射 SCK=4 MOSI=5 CS=6 DC=7 RST=21（待实物 continuity）。
...
# 本脚本是 release/CI 复现入口，不接受“本机刚好有另一个版本”。
...
if [ "$TFT_VER" != "2.5.44" ]; then
    echo "ERROR: TFT_eSPI 2.5.44 required, installed='${TFT_VER:-none}'" >&2
    echo "       arduino-cli lib install TFT_eSPI@2.5.44" >&2
    exit 2
fi
