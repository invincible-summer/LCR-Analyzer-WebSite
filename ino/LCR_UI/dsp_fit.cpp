...
//   [ Σs   Σc   N  ] [c]   [ Σx  ]
//
// 相位折叠：时间基准 t[k] = t0 + k·dt 中的大数值部分（t0 或 k·dt 很大时）
// 会放大 sin/cos 实参误差，这里统一折叠相位 θ[k] = ω·(t0 + k·dt) mod 2π。
// ============================================================================
...
static SineFitResult fitCore(const int16_t* x, size_t n, double w,
                             double dt, double t0)
{
    SineFitResult r = {0, 0, 0, 0, 0, false};
    if (!x || n < 8 || !(w > 0.0) || !(dt > 0.0) || !isfinite(t0))
        return r;

    // ---- 累加法方程矩阵与右端项 -------------------------------------------
    double Ss = 0, Sc = 0;        // Σsin, Σcos
    double Sss = 0, Scc = 0, Ssc = 0;
    double Ssx = 0, Scx = 0, Sx = 0;
    for (size_t k = 0; k < n; ++k) {
        double ph = w * (t0 + (double)k * dt);
        ph = fmod(ph, 2.0 * M_PI);           // 相位折叠（见文件头）
...
SineFitResult sineFit3(const int16_t* x, size_t n, double freqHz,
                       double sampleFreqHz)
{
    return fitCore(x, n, 2.0 * M_PI * freqHz, 1.0 / sampleFreqHz, 0.0);
}

SineFitResult sineFitTimed(const SampleSeries& s, double actualFreqHz)
{
    return fitCore(s.samples, s.count, 2.0 * M_PI * actualFreqHz, s.dt, s.t0);
}
