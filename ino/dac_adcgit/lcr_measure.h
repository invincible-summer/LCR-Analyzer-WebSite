...
#include "lcr_tone.h"
#include "freq_calc.h"
#include "lcr_diag.h"      // ★ 新增：诊断开关（须在 lcr_calib_core.h 之前可见）
#include "lcr_calib_core.h"
...
#define MUX_T_MASK ((1u << HC595_BIT_T3) | (1u << HC595_BIT_T2) | \
                    (1u << HC595_BIT_T1) | (1u << HC595_BIT_T0))
...
// ---- 档位类型 ----
enum VoltageRange : uint8_t { VOLT_X1 = 0, VOLT_X3 = 1, VOLT_X10 = 2, VOLT_X33 = 3 };
enum CurrentRange : uint8_t { CURR_X1 = 0, CURR_X3 = 1, CURR_X10 = 2, CURR_X33 = 3 };
enum TiaRange : uint8_t { TIA_22 = 0, TIA_100, TIA_330, TIA_1K, TIA_3K3, TIA_10K,
                          TIA_33K, TIA_100K, TIA_330K, TIA_1M };
...
// ================== 档位 → 物理值 / 显示名 ==================
static const char *lcr_volt_range_name(VoltageRange r) { return VOLT_RANGE_NAME[(uint8_t)r]; }
static const char *lcr_curr_range_name(CurrentRange r) { return CURR_RANGE_NAME[(uint8_t)r]; }
static const char *lcr_tia_range_name(TiaRange r)      { return TIA_RANGE_NAME[(uint8_t)r]; }
static double lcr_volt_gain_of(VoltageRange r) {
    uint8_t c = (uint8_t)r; return (c < 4) ? GAIN_OF_CODE[c] : 1.0;
}
static double lcr_curr_gain_of(CurrentRange r) {
    uint8_t c = (uint8_t)r; return (c < 4) ? GAIN_OF_CODE[c] : 1.0;
}
static double lcr_tia_ohm_of(TiaRange r) {
    uint8_t c = (uint8_t)r; if (c >= TIA_RANGE_COUNT) c = 0; return TIA_OHM[c];
}
...
static void s_mux_voltage_write(uint8_t code) {
    s_hc595_write(MUX_U_MASK, ((code & 0x2) ? (1u << HC595_BIT_U_MSB) : 0) |
                              ((code & 0x1) ? (1u << HC595_BIT_U_LSB) : 0));
}
static void s_mux_current_write(uint8_t code) {
    s_hc595_write(MUX_I_MASK, ((code & 0x2) ? (1u << HC595_BIT_I_MSB) : 0) |
                              ((code & 0x1) ? (1u << HC595_BIT_I_LSB) : 0));
}
static void s_mux_tia_write(uint8_t code) {
    s_hc595_write(MUX_T_MASK, ((code & 0x8) ? (1u << HC595_BIT_T3) : 0) |
                              ((code & 0x4) ? (1u << HC595_BIT_T2) : 0) |
                              ((code & 0x2) ? (1u << HC595_BIT_T1) : 0) |
                              ((code & 0x1) ? (1u << HC595_BIT_T0) : 0));
}
...
static ChanStats s_chan_stats(const uint16_t *buf, uint32_t n, double f) {
    ChanStats st;
    st.n = n;
    st.v_min = 99.f; st.v_max = -99.f; st.v_mean = 0.f; st.amp_v = 0.f;
...
static void s_si_fmt(char *out, size_t len, double v, const char *unit) {
    ...
    int idx = 4;
    double a = fabs(v);
    while (a >= 1000.0 && idx < 7) { a /= 1000.0; v /= 1000.0; idx++; }
    while (a > 0.0 && a < 1.0 && idx > 0) { a *= 1000.0; v *= 1000.0; idx--; }
...
static bool lcr_derive_z(const MeasResult &m, ZDerived &z) {
    ...
    if (fabs(phi_y) < LCR_RESISTIVE_TOL_DEG) { z.type = 'R'; z.cs = z.ls = z.cp = z.lp = NAN; }
    else if (z.B > 0.0) { z.type = 'C'; z.ls = z.lp = NAN; }
    else                { z.type = 'L'; z.cs = z.cp = NAN; }
    return true;
}
...
static void lcr_print_lcr_analysis(const MeasResult &m) { /* …原样保留… */ 
    ZDerived z;
    if (!lcr_derive_z(m, z)) { Serial.println("# LCR ERR: amplitude ratio invalid"); return; }
    ...
    if (z.type == 'R') {
        ...
        if (z.z_re < 0.0) Serial.println("# LCR WARN: negative R -> active DUT or sign config wrong");
    } else if (z.type == 'C') {
        s_si_fmt(s1, sizeof(s1), z.rp, "ohm"); s_si_fmt(s2, sizeof(s2), z.cp, "F");
        s_si_fmt(s3, sizeof(s3), z.rs, "ohm"); s_si_fmt(s4, sizeof(s4), z.cs, "F");
        ...
        if (z.rp < 0.0 || z.rs < 0.0) Serial.println("# LCR WARN: negative R -> active DUT or sign config wrong");
        if (z.D > 5.0) Serial.println("# LCR HINT: D>>1 -> loss dominates, treat as resistive DUT");
    } else {
        s_si_fmt(s1, sizeof(s1), z.rs, "ohm"); s_si_fmt(s2, sizeof(s2), z.ls, "H");
        s_si_fmt(s3, sizeof(s3), z.rp, "ohm"); s_si_fmt(s4, sizeof(s4), z.lp, "H");
        ...
        if (z.rs < 0.0) Serial.println("# LCR WARN: negative Rs -> active DUT or sign config wrong");
        if (z.D > 5.0) Serial.println("# LCR HINT: D>>1 -> loss dominates, treat as resistive DUT");
    }
}
...
static void lcr_print_sweep_table(const ZDerived *tab, uint8_t n) { /* 原样保留 */
    ...
    s_print_row("f_Hz", tab, n, [](const ZDerived &z){ return z.f; }, false);
    Serial.printf("# %-9s:", "type");
    for (uint8_t i = 0; i < n; i++) Serial.printf(" %c", tab[i].type);
    Serial.println();
    s_print_row("Zre_ohm", tab, n, [](const ZDerived &z){ return z.z_re; }, true);
    s_print_row("Zim_ohm", tab, n, [](const ZDerived &z){ return z.z_im; }, true);
    s_print_row("Zmag_ohm",tab, n, [](const ZDerived &z){ return z.z_mag; }, true);
    s_print_row("phiZ_deg",tab, n, [](const ZDerived &z){ return z.phi_z_deg; },false);
    s_print_row("D_tand", tab, n, [](const ZDerived &z){ return z.D; }, false);
    s_print_row("Q", tab, n, [](const ZDerived &z){ return z.Q; }, false);
    s_print_row("Rs_ohm", tab, n, [](const ZDerived &z){ return z.rs; }, true);
    s_print_row("Cs_F", tab, n, [](const ZDerived &z){ return z.cs; }, true);
    s_print_row("Ls_H", tab, n, [](const ZDerived &z){ return z.ls; }, true);
    s_print_row("Rp_ohm", tab, n, [](const ZDerived &z){ return z.rp; }, true);
    s_print_row("Cp_F", tab, n, [](const ZDerived &z){ return z.cp; }, true);
    s_print_row("Lp_H", tab, n, [](const ZDerived &z){ return z.lp; }, true);
    Serial.println("# note: Cs/Ls valid per type; nan = N/A; Rs=Zre, D=|Zre/Zim|");
    Serial.println("# ===================================================");
}

// ================== W 旁路模式 ==================
struct WPoint { double f, h_mag, h_db, phase_deg; };
